...
fetch('https://imdb-top-100-movies.p.rapidapi.com/', options)
	...
	.then(response => buildSelectedCards(response))
...
//     }
// ]
buildSelectedCards(response)

function buildSelectedCards(jsonData){
    moviesJson = jsonData
    console.log(moviesJson)
    buildList(moviesJson)
    $search.addEventListener('input', function (e) {
        e.preventDefault()
        if ($search.value) {
            console.log($search.value)
          const filteredMovies = moviesJson.filter((value) => value.title.toUpperCase().includes($search.value.toUpperCase()) )
...
function buildList(json){
    ...
    {
        let template = `
        <div class="col" onclick="checkPermit('detail.html?id=${json[i].id}')">
            <div class="card m-3">
                <div class="row g-0">
                    ...
                    <div class="col-xs-5 col-sm-6 col-md-6 ">
                        <div class="card-body">
                            <h5 class="card-title">Title : ${json[i].title} (Rank :${json[i].rank})</h5>
                            <p class="card-text">
                                <b>Rating:</b> ${json[i].rating}
                            </p>
                            <p class="card-text">
                                <b>Released year:</b> ${json[i].year}
                            </p>
                            <p class="card-text">
                                <b>Genre:</b> ${json[i].genre.join(', ')}
                            </p>
                            <p class="card-text">
                                <b>Director:</b> ${json[i].director.join(', ')}
                            </p>
